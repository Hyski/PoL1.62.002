...
bool AssaultTechEnemyNode::thinkFixed(state_type* st)
{
	...
	if(AIAPI::getEnemiesVisibleByEntity(m_entity, &enemies))
	{
		...
		{
			AIAPI::print(m_entity, "|AT|thinkFixed: я увидела врага!");
	...
	if(m_shooted)
	{
		AIAPI::print(m_entity, "|AT|thinkFixed: в меня стреляли - будем преследовать");
	...
	if(m_enemy_id)
	{
		AIAPI::print(m_entity, "|AT|thinkFixed: мне сообщили о враге");
		// сообщили
		m_enemy_id = 0;
	...
	if(m_activity_observer.getLastEvent() == ActivityObserver::EV_MOVEPNTS_EXPIRIED)
	{
		...
		AIAPI::print(m_entity, "|AT|thinkFixed: не хватило мувпоинтов");
	...
	AIAPI::print(m_entity, "|AT|thinkFixed: идем в заданную точку");
	PathUtils::CalcPassField(AIAPI::getPtr(m_entity),	0, 0, 0);
	pnt_vec_t path;
	PathUtils::CalcPath(AIAPI::getPtr(m_entity), m_target_pnt, path);
	if(path.empty())
	...
	// больше делать нечего, отдадим ход
	m_turn_state = TS_END;
	return false;
	//// можно идти
	//m_activity = ActivityFactory::CreateMove(AIAPI::getPtr(m_entity),	path, ActivityFactory::CT_ENEMY_MOVE);
...
bool AssaultTechEnemyNode::thinkPatrol(state_type* st)
{
	...
	if(AIAPI::getEnemiesVisibleByEntity(m_entity, &enemies))
	{
		...
		{
			AIAPI::print(m_entity, "|AT|thinkPatrol: я увидела врага!");
			// выбралa самого опасного врага
			// пошлю сообщение своей команде
			getSubteam()->sendMessage(this,	AIEnemyNode::MT_ENEMYSPOTTED, &enemy, assault_tech_enemy_dist);
			// переход в состояние атаки
	...
	if(m_shooted)
	{
		AIAPI::print(m_entity, "|AT|thinkPatrol: в меня стреляли - будем преследовать");
		// в меня или рядом со мной стреляли
		m_shooted = false;
		// выберем точку в радиусе 3 хексов от существа, которое стреляло
		// пойдем туда и перейдем при этом в состояние преследования
	...
	if(m_enemy_id)
	{
		AIAPI::print(m_entity, "|AT|thinkPatrol: мне сообщили о враге");
		// сообщили
		m_enemy_id = 0;
		// выберем точку в радиусе 1 хекса от врага
		// пойдем туда и перейдем при этом в состояние преследования
	...
	{
		AIAPI::print(m_entity, "|AT|thinkPatrol: дошла до заданной точки - выбирает новую");
		// рассчитаем поле проходимости
		pnt_vec_t allhexes;
		...
		if(allhexes.empty())
		{
			...
			AIAPI::print(m_entity, "|AT|thinkPatrol: мне некуда ходить!!!");
			m_turn_state = TS_END;
			return false;
		}
	...
	{
		// не хватило мувпоинтов
		AIAPI::print(m_entity, "|AT|thinkPatrol: не хватило мувпоинтов");
		m_activity_observer.clear();
		m_turn_state = TS_END;
		return false;
	}
	...
	// 6. идем в заданную точку
	AIAPI::print(m_entity, "|AT|thinkPatrol: идем в заданную точку");
	PathUtils::CalcPassField(AIAPI::getPtr(m_entity),	0, 0, 0);
	pnt_vec_t path;
	PathUtils::CalcPath(AIAPI::getPtr(m_entity), m_target_pnt, path);
	if(path.empty())
...
bool AssaultTechEnemyNode::thinkAttack(state_type* st)
{
	...
	{
		AIAPI::print(m_entity, "|AT|thinkAttack: нет оружия - будем преследовать");
		m_mode = ATEM_PURSUIT;
		m_turn_state = TS_INPROGRESS;
		m_activity_observer.clear();
		return false;
	...
	if(!enemy)
	{
		...
		AIAPI::print(m_entity, "|AT|thinkAttack: врагов нет - будем преследовать последнего");
		m_mode = ATEM_PURSUIT;
		m_turn_state = TS_INPROGRESS;
	...
	if(m_turn_state == TS_START)
	{
		...
		AIAPI::print(m_entity, "|AT|thinkAttack: выбираем лучшую точку и идем в нее");
		...
		int movpoints = AIAPI::getMovePoints(m_entity);
		// вычтем количество мувпоинтов необходимых для выстрела
		movpoints -= AIAPI::getShootMovePoints(m_entity,	/*забьет на этот параметр*/SHT_AIMSHOT);
		// вычтем мувпоинты на поворот к врагу
		movpoints -= AIUtils::CalcMps4EntityRotate(PIm2);
		int steps = AIAPI::getSteps(m_entity, movpoints);
		if(!steps)
	...
	if(m_activity_observer.getLastEvent() == ActivityObserver::EV_MOVEPNTS_EXPIRIED)
	{
		...
		AIAPI::print(m_entity, "|AT|thinkAttack: закончились мувпоинты");
		m_activity_observer.clear();
		m_turn_state = TS_END;
		return false;
	}
	...
	{
		AIAPI::print(m_entity, "|AT|thinkAttack: сработала реакция или увидел нового врага");
		m_activity_observer.clear();
	...
	{
		// закончились - закончим ход
		AIAPI::print(m_entity, "|AT|thinkAttack: нет линии огня");
		m_activity_observer.clear();
		...
		if(diff_list.empty())
		{
			...
			AIAPI::print(m_entity, "|AT|thinkAttack: подойдем поближе");
			// рассчитаем поле проходимости
	...
	AIAPI::print(m_entity, "|AT|thinkAttack: будем стрелять, если это возможно!");
...
bool AssaultTechEnemyNode::thinkPursuit(state_type* st)
{
	...
	if(AIAPI::getEnemiesVisibleByEntity(m_entity, &enemies))
	{
		...
		{
			AIAPI::print(m_entity, "|AT|thinkPursuit: я увиделa врага!");
			// выбралa самого опасного врага
			// пошлю сообщение своей команде
			getSubteam()->sendMessage(this,	AIEnemyNode::MT_ENEMYSPOTTED, &enemy, assault_tech_enemy_dist);
			// переход в состояние атаки
	...
	{
		// не хватило мувпоинтов
		AIAPI::print(m_entity, "|AT|thinkPursuit: не хватило мувпоинтов");
		m_activity_observer.clear();
		m_turn_state = TS_END;
		return false;
	}
	...
	if(m_target_pnt == AIAPI::getPos2(m_entity))
	{
		...
		AIAPI::print(m_entity, "|AT|thinkPursuit: никого не догналa - будем патрулировать");
	...
	AIAPI::print(m_entity, "|AT|thinkPursuit: идем в заданную точку");
	PathUtils::CalcPassField(AIAPI::getPtr(m_entity),	0, 0, 0);
	pnt_vec_t path;
	PathUtils::CalcPath(AIAPI::getPtr(m_entity), m_target_pnt, path);
	if(path.empty())
